...
def main(columns, alias_experiment, dataset="cipi"):
    if dataset == "cipi":
        ...
        if FEATURES == "jsymbolic":
            ...
            features = {v["id"]: v for v in utils.load_json("../features/basic-CIPI.json")}

    final_acc9, final_acc3, final_acc1, final_mse, final_best_selection = [], [], [], [], []

    best_acc_val, best_selection = 0, ""
    results = {str(idx): {} for idx in range(5)}
    for split in range(5):
        ...
        results[str(split)] = {"paths": list(ids_test), "labels": y_test.tolist(), "preds": pred_test.detach().cpu().tolist()}
    utils.save_json(results, f"legacy/prediction_results_{alias_experiment}.json")
...
def parse_args():
    ...
    # Add arguments
    parser.add_argument("--alias_experiment", type=str, default="rubricnet", help="Alias name for the experiment.")
    ...
    parser.add_argument("--weight_decay", type=float, default=0.1, help="Learning rate for the optimizer. Default is 0.1.")
